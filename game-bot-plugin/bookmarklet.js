/**
 * Iceman Runner Bot – standalone bookmarklet / userscript core
 *
 * This file is the complete self-contained bot. It is used two ways:
 *   1. As a bookmarklet on iOS Safari  (see install-ios.html for setup)
 *   2. As the body of userscript.js    (auto-runs via Userscripts app)
 *
 * Differences from content.js:
 *   • Runs directly in page-JS context, so window.game/runner accessible without shim.
 *   • Touch-first input (TouchEvent API) with keyboard as fallback.
 *   • iOS-optimised panel: pinned to bottom-centre, large tap targets, touch-draggable.
 *   • Idempotent: tapping the bookmarklet again toggles the bot on/off.
 *   • Game-object hook probes window globals directly on every jump attempt.
 */
(function () {
  "use strict";
  ...
  /* ── game-object hook (direct window access in page context) ───────────── */
  ...
  // Simulate a tap at the bottom-centre of the canvas (where player is)
  function tapCanvas(down) {
    if (!S.canvas) return;
    const r = S.canvas.getBoundingClientRect();
    const cx = r.left + r.width  * 0.5;
    const cy = r.top  + r.height * 0.72; // player zone
    ...
    // Pointer path (desktop / Chrome Android)
    try {
      S.canvas.dispatchEvent(new PointerEvent(down ? "pointerdown" : "pointerup", { bubbles: true, cancelable: true, clientX: cx, clientY: cy, isPrimary: true }));
    } catch (_) {}
  ...
  // Simulate a quick downward swipe (duck gesture for mobile)
  function swipeDown() {
    if (!S.canvas || typeof TouchEvent === "undefined") return;
    try {
      const r = S.canvas.getBoundingClientRect();
      const cx = r.left + r.width * 0.5;
      const y1 = r.top + r.height * 0.35;
      const y2 = r.top + r.height * 0.65;
      const id = Date.now() & 0xffff;
      const mkTouch = (y) => new Touch({ identifier: id, target: S.canvas, clientX: cx, clientY: y, screenX: cx, screenY: y, pageX: cx + scrollX, pageY: y + scrollY, radiusX: 2, radiusY: 2, rotationAngle: 0, force: 1 });
      const t1 = mkTouch(y1);
      ...
      setTimeout(() => {
        const t2 = mkTouch(y2);
        S.canvas.dispatchEvent(new TouchEvent("touchend",   { bubbles: true, cancelable: true, touches: [],   targetTouches: [],   changedTouches: [t2] }));
      }, 60);
    } catch (_) {}
  }
  ...
  function pressJump() {
    ...
    callGameJump();                                          // direct game call
    key("keydown", " ",        "Space",   32);              // keyboard
    key("keydown", "ArrowUp",  "ArrowUp", 38);
    tapCanvas(true);                                         // touch/pointer
    setTimeout(() => {
      key("keyup", " ",       "Space",   32);
  ...
  function createPanel() {
    ...
    if (mob) {
      // Bottom strip – minimal footprint, large touch button
      p.style.cssText = [
...
<button id="__rbb__" style="
  padding:11px 22px;cursor:pointer;border:none;border-radius:10px;
  background:linear-gradient(135deg,#1c8,#0a5);color:#fff;
  font:700 13px monospace;touch-action:manipulation;
  min-width:90px;flex-shrink:0">▶ Start</button>`;
    } else {
      p.style.cssText = [
...
<div style="margin-top:10px;color:#333;font-size:10px;line-height:1.8">
  60 fps · adaptive scan · jump+duck<br>air-guard · auto-restart ✓
</div>`;
    }
    ...
    p.addEventListener("touchmove", e => {
      if (!dragging || e.target === S.btn) return;
      e.preventDefault();
      const t = e.touches[0];
      p.style.left      = (sl + t.clientX - sx) + "px";
      p.style.top       = (st + t.clientY - sy) + "px";
      p.style.bottom    = "auto";
      p.style.right     = "auto";
      p.style.transform = "none";
    }, { passive: false });
    p.addEventListener("touchend", () => { dragging = false; }, { passive: true });

    // Mouse drag (desktop head)
  ...
  function updateStatus() {
    ...
    setStatus(`Running ${time} · ${S.jumpCount}j`);
  }
  ...
  function updateInfo() {
    ...
    S.infoEl.textContent = S.tainted
      ...
      : `${S.canvas.width}×${S.canvas.height} · pixel+duck`;
  }
  ...
  // Probe for game object once DOM is stable
